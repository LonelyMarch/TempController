...
void lv_text_ap_proc(const char* txt, char* txt_out)
{
    ...
    while (i < txt_length)
    {
        ...
        if (lv_text_is_arabic_vowel(ch_enc[i]))
        {
        ...
        else if (lv_text_is_arabic_vowel(ch_enc[i + 1]))
        {
            // Next character is a vowel
            idx_next = lv_ap_get_char_index(ch_enc[i + 2]);
        ...
        if (conjunction_to_previous && conjunction_to_next)
            ...
            ch_fin[j] = ap_chars_map[index_current].char_end_form + ap_chars_map[index_current].
...
static uint32_t lv_ap_get_char_index(uint16_t c)
{
    for (uint8_t i = 0; ap_chars_map[i].char_end_form; i++)
    {
        if (c == (ap_chars_map[i].char_offset + LV_AP_ALPHABET_BASE_CODE))
            ...
            || c == (ap_chars_map[i].char_end_form + ap_chars_map[i].char_beginning_form_offset)
            //is it a Beginning form
