                                        ...
                                        lv_3dpoint_t* collision_point)
{
    ...
    fastgltf::math::fvec3 plane_normal = fastgltf::math::fvec3(plane->direction.x, plane->direction.y,
                                                               plane->direction.z);
    ...
    if (fabs(denom) > 1e-6)
    {
        ...
        if (t >= 0)
        {
            /* Intersection occurs ahead of the ray origin */
...
lv_3dplane_t lv_gltf_get_current_view_plane(lv_obj_t* obj, float distance)
{
    ...
    auto forward = fastgltf::math::fvec3(viewer->view_matrix[0][2], viewer->view_matrix[1][2],
                                         viewer->view_matrix[2][2]);
    ...
    const auto& iterate_callback = [&](fastgltf::Node& node, const fastgltf::math::fmat4x4& matrix)
    {
        ...
        for (size_t mp = 0; mp < model->asset.meshes[mesh_index].primitives.size(); mp++)
        {
            ...
            lv_opengl_shader_params_t frag_shader{
                "__MAIN__.frag", (lv_opengl_shader_define_t*)defines.data, lv_array_size(&defines)
            };
            lv_opengl_shader_params_t vert_shader{
                "__MAIN__.vert", (lv_opengl_shader_define_t*)defines.data, lv_array_size(&defines)
            };

            lv_opengl_shader_program_t* program = lv_opengl_shader_manager_compile_program_best_version(
                &viewer->shader_manager,
                &frag_shader,
