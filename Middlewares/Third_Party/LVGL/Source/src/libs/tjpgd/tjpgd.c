        ...
        else
        {
            ...
            if (JD_FORMAT != 2 || !cmp)
            {
                ...
                if (z == 1 || (JD_USE_SCALE &&
                    ...
                    3))
                {
                    /* If no AC element or scale ratio is 1/8, IDCT can be omitted and the block is filled with DC value */
...
)
{
    ...
    memset(jd, 0, sizeof(
               JDEC));
    /* Clear decompression object (this might be a problem if machine's null pointer is not all bits zero) */
