                                 ...
                                 uint32_t unicode_letter_next)
{
    ...
    if (entry == NULL)
    {
        if (!dsc->cache_size)
        {
            /* no cache, do everything directly */
            ...
            *dsc_out = search_key.glyph_dsc;
            ...
            {
                int g2 = stbtt_FindGlyphIndex(&dsc->info, (int)unicode_letter_next);
                /* not using cache, only do glyph id lookup */
                if (g2)
                {
    ...
    *dsc_out = data->glyph_dsc;
    ...
    if (font->kerning == LV_FONT_KERNING_NORMAL &&
        unicode_letter_next != 0)
    {
        /* check if we need to do any kerning calculations */
...
{
    /*Init cache*/
    dsc->glyph_cache = lv_cache_create(&lv_cache_class_lru_rb_count, sizeof(tiny_ttf_glyph_cache_data_t),
                                       dsc->cache_size,
                                       (lv_cache_ops_t)
    {
        .
...
static bool tiny_ttf_glyph_cache_create_cb(tiny_ttf_glyph_cache_data_t* node, void* user_data)
{
    ...
    if (dsc->kerning != LV_FONT_KERNING_NORMAL)
    {
        /* calculate default advance */
