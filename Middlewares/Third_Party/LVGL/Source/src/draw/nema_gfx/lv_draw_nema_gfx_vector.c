...
static void task_draw_cb(void* ctx, const lv_vector_path_t* path, const lv_vector_path_ctx_t* dsc)
{
    ...
    {
        nema_ops[i] = ops[i] == LV_VECTOR_PATH_OP_MOVE_TO
                          ? NEMA_VG_PRIM_MOVE
                          : ops[i] == LV_VECTOR_PATH_OP_LINE_TO
                          ? NEMA_VG_PRIM_LINE
                          : ops[i] == LV_VECTOR_PATH_OP_QUAD_TO
                          ? NEMA_VG_PRIM_BEZIER_QUAD
                          : ops[i] == LV_VECTOR_PATH_OP_CUBIC_TO
    ...
    nema_vg_set_quality(path->quality == LV_VECTOR_PATH_QUALITY_LOW
                            ? NEMA_VG_QUALITY_FASTER
                            : path->quality == LV_VECTOR_PATH_QUALITY_MEDIUM
    ...
    if (dsc->fill_dsc.opa)
    {
        nema_vg_set_fill_rule(dsc->fill_dsc.fill_rule == LV_VECTOR_FILL_NONZERO
                                  ? NEMA_VG_FILL_NON_ZERO
                                  :
    ...
    if (dsc->stroke_dsc.opa)
    {
        ...
        uint8_t cap = dsc->stroke_dsc.cap == LV_VECTOR_STROKE_CAP_BUTT
                          ? NEMA_VG_CAP_BUTT
                          : dsc->stroke_dsc.cap == LV_VECTOR_STROKE_CAP_SQUARE
                          ? NEMA_VG_CAP_SQUARE
                          :
                          /*LV_VECTOR_STROKE_CAP_ROUND*/ NEMA_VG_CAP_ROUND;
        nema_vg_stroke_set_cap_style(cap, cap);
        uint8_t join = dsc->stroke_dsc.join == LV_VECTOR_STROKE_JOIN_MITER
                           ? NEMA_VG_JOIN_MITER
                           : dsc->stroke_dsc.join == LV_VECTOR_STROKE_JOIN_BEVEL
                           ? NEMA_VG_JOIN_BEVEL
                           :
