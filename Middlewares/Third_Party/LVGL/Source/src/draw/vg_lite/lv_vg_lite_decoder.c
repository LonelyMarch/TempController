...
static lv_result_t decoder_info(lv_image_decoder_t* decoder, lv_image_decoder_dsc_t* dsc, lv_image_header_t* header)
{
    ...
    if (header->flags & LV_IMAGE_FLAGS_COMPRESSED)
    {
        ...
        LV_LOG_WARN("NOT Supported compressed flags: %d, format: %d, type: %d, src: %p (%s)",
                    header->flags, header->cf, src_type, dsc->src,
                    src_type == LV_IMAGE_SRC_FILE ? (const char*)dsc->src : "variable");
        return LV_RESULT_INVALID;
    }
    ...
    case LV_IMAGE_SRC_FILE:
        {
            ...
            {
                LV_LOG_ERROR("read %s lv_image_header_t failed, res: %d, br: %" LV_PRIu32, (const char*)dsc->src,
                             fs_res, header_br);
                lv_fs_close(&file);
                return LV_RESULT_INVALID;
            }
