        ...
        else if (mask_buf && opa >= LV_OPA_MAX)
        {
            if (LV_RESULT_INVALID == LV_DRAW_SW_AL88_BLEND_NORMAL_TO_RGB888_WITH_MASK(dsc, dest_px_size))
            {
                for (y = 0; y < h; y++)
                {
                    for (dest_x = 0, src_x = 0; src_x < w; dest_x += dest_px_size, src_x++)
                    {
                        lv_color_8_24_mix(src_buf_al88[src_x].lumi, &dest_buf_u8[dest_x], LV_OPA_MIX2(
        ...
        else if (mask_buf && opa < LV_OPA_MAX)
        {
            if (LV_RESULT_INVALID == LV_DRAW_SW_AL88_BLEND_NORMAL_TO_RGB888_MIX_MASK_OPA(dsc, dest_px_size))
            {
                for (y = 0; y < h; y++)
                {
                    for (dest_x = 0, src_x = 0; src_x < w; dest_x += dest_px_size, src_x++)
                    {
                        lv_color_8_24_mix(src_buf_al88[src_x].lumi, &dest_buf_u8[dest_x], LV_OPA_MIX3(
                                              src_buf_al88[src_x].alpha,
                                              mask_buf[src_x], opa));
                    }
                    dest_buf_u8 += dest_stride;
    ...
    if (dsc->blend_mode == LV_BLEND_MODE_NORMAL)
    {
        ...
        if (mask_buf && opa >= LV_OPA_MAX)
        {
            if (LV_RESULT_INVALID == LV_DRAW_SW_RGB888_BLEND_NORMAL_TO_RGB888_WITH_MASK(dsc, dest_px_size, src_px_size))
            {
                ...
                for (y = 0; y < h; y++)
                {
                    for (mask_x = 0, dest_x = 0, src_x = 0; dest_x < w; mask_x++, dest_x += dest_px_size, src_x +=
                         src_px_size)
                    {
        ...
        if (mask_buf && opa < LV_OPA_MAX)
        {
            if (LV_RESULT_INVALID == LV_DRAW_SW_RGB888_BLEND_NORMAL_TO_RGB888_MIX_MASK_OPA(
                dsc, dest_px_size, src_px_size))
            {
                uint32_t mask_x;
                for (y = 0; y < h; y++)
                {
                                                                     ...
                                                                     uint32_t dest_px_size)
{
    ...
    if (dsc->blend_mode == LV_BLEND_MODE_NORMAL)
    {
            ...
            if (LV_RESULT_INVALID == LV_DRAW_SW_ARGB8888_PREMULTIPLIED_BLEND_NORMAL_TO_RGB888(dsc, dest_px_size))
            {
                for (y = 0; y < h; y++)
                {
                    for (dest_x = 0, src_x = 0; src_x < w; dest_x += dest_px_size, src_x++)
                    {
                        ...
                        lv_color_24_24_mix_premult((const uint8_t*)&src_buf_c32[src_x], &dest_buf[dest_x],
                                                   src_buf_c32[src_x].alpha);
                    }
                    dest_buf += dest_stride;
                    src_buf_c32 = drawbuf_next_row(src_buf_c32, src_stride);
        ...
        {
            if (LV_RESULT_INVALID == LV_DRAW_SW_ARGB8888_PREMULTIPLIED_BLEND_NORMAL_TO_RGB888_WITH_OPA(
                dsc, dest_px_size))
            {
                for (y = 0; y < h; y++)
                {
                    for (dest_x = 0, src_x = 0; src_x < w; dest_x += dest_px_size, src_x++)
                    {
                        ...
                        if (src_pixel.alpha > 0)
                        {
                            ...
                            lv_color_24_24_mix((const uint8_t*)&src_pixel, &dest_buf[dest_x],
                                               LV_OPA_MIX2(src_pixel.alpha, opa));
                        }
                    }
                    dest_buf += dest_stride;
                    src_buf_c32 = drawbuf_next_row(src_buf_c32, src_stride);
        ...
        {
            if (LV_RESULT_INVALID == LV_DRAW_SW_ARGB8888_PREMULTIPLIED_BLEND_NORMAL_TO_RGB888_WITH_MASK(
                dsc, dest_px_size))
            {
                for (y = 0; y < h; y++)
                {
                    for (dest_x = 0, src_x = 0; src_x < w; dest_x += dest_px_size, src_x++)
                    {
                        ...
                        if (src_pixel.alpha > 0)
                        {
                            ...
                            src_pixel.blue = (src_pixel.blue * reciprocal) >> 8;
                            lv_color_24_24_mix((const uint8_t*)&src_pixel, &dest_buf[dest_x],
                                               LV_OPA_MIX2(src_pixel.alpha, mask_buf[src_x]));
                        }
                    }
                    dest_buf += dest_stride;
                    src_buf_c32 = drawbuf_next_row(src_buf_c32, src_stride);
        ...
        {
            if (LV_RESULT_INVALID == LV_DRAW_SW_ARGB8888_PREMULTIPLIED_BLEND_NORMAL_TO_RGB888_MIX_MASK_OPA(
                dsc, dest_px_size))
            {
                for (y = 0; y < h; y++)
                {
