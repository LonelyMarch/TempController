...
void lv_obj_get_scrollbar_area(lv_obj_t* obj, lv_area_t* hor_area, lv_area_t* ver_area)
{
    ...
    if (ver_draw && content_h)
    {
        ...
        sb_h = LV_MAX(length > 0 ? length : sb_h, SCROLLBAR_MIN_SIZE);
        /*Style-defined size, calculated size, or minimum size*/
    ...
    if (hor_draw && content_w)
    {
        ...
        sb_w = LV_MAX(length > 0 ? length : sb_w, SCROLLBAR_MIN_SIZE);
        /*Style-defined size, calculated size, or minimum size*/
