...
void lv_obj_style_set_disabled(lv_obj_t* obj, const lv_style_t* style, lv_style_selector_t selector, bool dis)
{
    ...
    LV_LOG_WARN("%p style was not found on %p widget with %6" LV_PRIx32 " selector", (void*)style, (void*)obj,
                selector);
}
...
static bool trans_delete(lv_obj_t* obj, lv_part_t part, lv_style_prop_t prop, trans_t* tr_limit)
{
    ...
    while (tr != NULL)
    {
        ...
        if (tr->obj == obj && (part == tr->selector || part == LV_PART_ANY) && (prop == tr->prop || prop ==
            LV_STYLE_PROP_ANY))
        {
...
static void trans_anim_cb(void* _tr, int32_t v)
{
    ...
    for (i = 0; i < obj->style_cnt; i++)
    {
        ...
        default:
            ...
            else value_final.num = tr->start_value.num + ((int32_t)(
                (int32_t)(tr->end_value.num - tr->start_value.num) * v) >> 8);
            break;
        }
